...
class ActivitiesViewController: UIViewController {
    ...
    private func fetchFromCoreData() {
        ...
        let fetching = Iughj.fetchRequest()
        ...
        do {
            ...
            let activityModels: [ActivitiesTableViewModel] = raw.map {
                ...
                return ActivitiesTableViewModel(distance: activity.distance, duration: activity.duration, activityTitle: activity.type ?? "", timeAgo: activity.date ?? Date(), icon: img, startTime: activity.start ?? "", endTime: activity.end ?? "")
            }
