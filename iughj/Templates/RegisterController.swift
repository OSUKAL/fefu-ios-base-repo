...
class RegisterController: UIViewController,  UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func didRegisterBtnTap(_ sender: Any) {
        ...
        do {
            let request = try AuthRegUrlManager.encoder.encode(preRequest)
            AuthRegUrlManager.reg(request) { user in
                print(user.token)
                AuthRegDataManager.intance.saveUser(login: login, password: password, pabKey: user.token)
